...
create or replace view cuentaNIF as
select count(NIF_INV) as nNIF
from COLABORACIONES
group by COD_EXP;
...
insert into PUNTUACIONES values ((select NIF
                                 from CATADORES 
                                 where substring(NIF, 1, 1) in ('K','L','M','X','Y','Z')),
(select CODIGO
from ASPECTOS 
where IMPORTANCIA='Baja'), (select COD_EXP
                            from COLABORACIONES
                            group by COD_EXP
                            having count(NIF_INV)=(select max(nNIF)
                                                   from cuentaNIF)),
(select CODIGO
from VERSIONES 
...
update VERSIONES
set FECHA_PRUEBA=FECHA_PRUEBA + interval '2' month
...
4. Muestra aquellos ingredientes preparados a partir de más 
de dos ingredientes básicos y que han sido utilizados en más 
de una versión de un mismo experimento.
...
where NIF in (select distinct NIF_CAT
              ...
              where (COD_VERS, COD_EXP) in (select CODIGO, COD_EXP
                                            from VERSIONES
                                            where extract(month from FECHA_PRUEBA) in ('05', '06', '09')));
...
create or replace view mesFechaExperimento as
select CODIGO, extract(month from FECHA_FIN) as MES_EXP
from EXPERIMENTOS;

create or replace view mesActual as
select extract(month from now()) as MES_ACTUAL;

select NOMBRE
from INGREDIENTES
where CODIGO not in (select distinct iv.COD_ING
                        from INGREDIENTES_POR_VERSION iv, mesFechaExperimento me, mesActual ma
                        where iv.COD_EXP=me.CODIGO
                        and cast(me.MES_EXP as integer)-cast(ma.MES_ACTUAL as integer)<4)
and CODIGO not in (select COD_ING_BASE
                from COMPOSICION_ING_PREPARADOS
                where COD_ING_FINAL in (select distinct iv.COD_ING
                        from INGREDIENTES_POR_VERSION iv, mesFechaExperimento me, mesActual ma
                        where iv.COD_EXP=me.CODIGO
                        and cast(me.MES_EXP as integer)-cast(ma.MES_ACTUAL as integer)<4));
