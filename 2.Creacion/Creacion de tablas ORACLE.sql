...
(
	COD_ING_BASE varchar2(3),
	COD_ING_FINAL varchar2(3),
	...
	constraint CANTIDAD_COMP_FORMAT check (regexp_like (CANTIDAD, '^[1-8]?[0-9]{1}([\.]([0-9]){0,1}[0-9])?[UGL]$')),
	constraint NULO_CANTIDAD check (CANTIDAD is not null)
);
...
(
	...
	constraint NIF_INV_FORMAT check (regexp_like (NIF,'^[0-9]{8}[A-Z]$') or regexp_like(NIF, '^[KLMXYZ][0-9]{7}[A-Z]$')),
...
(
    ...
    constraint COD_EXP_FORMAT check (regexp_like (CODIGO, '^[A-Z][0-9]{4}-[A-Z]$')),
...
(
    ...
    constraint COD_VERS_FORMAT check (regexp_like (CODIGO, '^[0-9]{1,2}\.{1}[0-9]{1,2}(\.[0-9]{1,2})*$') and CODIGO not like '%.'),
...
(
    ...
    constraint CANTIDAD_FORMAT check (regexp_like(CANTIDAD, '^[1-8]?[0-9]{1}([\.]([0-9]){0,1}[0-9])?[UGL]$')),
...
(
    ...
    constraint NIF_CAT_FORMAT check (regexp_like (NIF,'^[0-9]{8}[A-Z]$') or regexp_like(NIF, '^[KLMXYZ][0-9]{7}[A-Z]$')),
...
(
    ...
    constraint NULO_VALOR_PUNT check (VALOR is not null)
);
